...
public class CameraFollow : MonoBehaviour
{
    ...
    private bool IsTooFar(GameObject origin, GameObject toCheck) //Check if the enemy is on the border of the screen
    {
        ...
        if ((pointOnScreen.x < Screen.width * 0.10f) || (pointOnScreen.x > Screen.width * 0.90f) || //if the enemy if within 10% on the edge of the screen, return "too far"
            (pointOnScreen.y < Screen.height * 0.10f) || (pointOnScreen.y > Screen.height * 0.9f))
        {
            Debug.Log("Istoofar: " + toCheck.name);
            return true;
        }
        else if((pointOnScreen.x > Screen.width * 0.25f) && (pointOnScreen.x < Screen.width * 0.75f) && //if the enemy if within 75% inside the screen, return "no too far"
                (pointOnScreen.y > Screen.height * 0.25f) && (pointOnScreen.y < Screen.height * 0.75f))
        {
            return false;
        }
